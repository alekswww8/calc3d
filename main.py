...
try:
    ...
    # --- ЭТОТ БЛОК ПЕРЕХВАТИТ ОШИБКУ И ПОКАЖЕТ ЕЕ НА ЭКРАНЕ ---
    from kivy.app import App
    from kivy.uix.label import Label
    from kivy.uix.scrollview import ScrollView
    ...
    class CrashApp(App):
        def build(self):
            ...
            lbl.bind(width=lambda *x: lbl.setter('text_size')(lbl, (lbl.width - 20, None)),
                     texture_size=lambda *x: lbl.setter('height')(lbl, lbl.texture_size[1] + 40))
            sv.add_widget(lbl)
            return sv
    CrashApp().run()
